...
import hueristic from "../hueristic";
import PriorityQueue from "../priorityQueue";
import reconstructPath from "./reconstructPath";

export default async function aStar(grid) {
	...
	fScore[startCell.id] = hueristic(startCell, endCell);
	...
	while (!openSet.isEmpty()) {
		...
		for (const neighbor of current.neighbors) {
			...
			if (tempGScore < gScore[neighbor.id]) {
				...
				fScore[neighbor.id] = tempGScore + hueristic(neighbor, endCell);

				if (openSetHash.has(neighbor)) continue;

				count++;

				openSet.add(fScore[neighbor.id], count, neighbor);
